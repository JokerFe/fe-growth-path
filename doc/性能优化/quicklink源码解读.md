...
`quicklink`是前端性能优化的一种手段，使用户后续页面的**第一次访问加快**。它检测页面的滚动，可以**预加载**出现在视窗内的页面链接，预加载下一页的资源，**提高用户体验**。是一种**以用户网络流量来换取体验**的手段。同时，也通过它的源码熟悉前端加载资源的优化方式。**PWA技术是优化用户的二次访问效率和体验**。
